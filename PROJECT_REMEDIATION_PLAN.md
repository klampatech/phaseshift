...
| 3.1 — Biomes | ✅ Done | Working tree | Walking from Forest into Crystal Cavern visibly tints the sky to purple (the §3.1 acceptance). New pure module `src/world/biome.js` with `BIOME_TINTS` (8 entries: `{ color: [r,g,b], fogDensity }` — Forest 0.006, Deep Void 0.025, Desert 0.004, Phase Nexus 0.018, etc), `biomeTint(id)` / `biomeLabel(id)` / `biomeFogDensity(id)` (defensive: out-of-range / NaN → Forest), `lerpBiomeTints(from, to, t)` (component-wise RGB lerp + scalar density lerp, clamps `t` to `[0, 1]`, handles NaN), `biomeTransitionDuration()` (returns 0.5 — the §3.1 smooth-fade value), plus re-exports of `BIOME_FOREST` … `BIOME_PHASE_NEXUS` + `BIOME_NAMES`. `src/render/renderer.js` extends `createSkybox` with `biomeTint` + `phaseTint` `THREE.Vector3` uniforms; fragment shader multiplies both into the base gradient (`gl_FragColor = vec4(base * biomeTint * phaseTint, 1.0)` — the §3.1 "phase × biome" formula). Skybox mesh tagged `sky.name = 'skybox'` with `setBiomeTint` / `setPhaseTint` convenience methods. `Renderer` class exposes `setBiomeTint` / `setPhaseTint` forwarders (look up skybox by name). `src/ui/hud.js` extends constructor + `update(phaseManager, physicsManager, world)` — the new `world` parameter is plumbed through at all 3 call sites — to query `world.getBiome(playerPos.x, playerPos.z)` and update `#biome-info` text on the change edge (`_lastBiomeId !== newBiomeId`); DOM write only fires on biome transitions. `main.js` adds module-level `currentBiomeId` / `currentBiomeTint` / `targetBiomeTint` / `biomeTransitionTimer` state + `tickBiomesPerFrame(dt)` (reads `world.getBiome`, resets timer on change edge, lerps via `lerpBiomeTints`, drives `scene.background.setRGB` + `scene.fog.color.setRGB` + `scene.fog.density` (clamped to `biomeFogDensity(currentBiomeId)` on last frame to avoid lerp float slop) + `lighting.phaseLight.color.setRGB` + `renderer.setBiomeTint`). `onPhaseChanged` drives the new `renderer.setPhaseTint` from `parseHexColor(colors[phase])`. Phase-vs-biome is multiplicative, not destructive — the phase color is applied first (Phase 2.1 regression lock), then the biome tint is multiplied on top. New debug hooks: `__phaseShifter__.forceBiome(biomeId)` (rejects bad input with `{ ok: false, reason }`; pins player), `.getCurrentBiomeId()`, `.getCurrentBiomeTint()`, `.lerpBiomeTints(from, to, t)` (pass-through), `.biomeLabel(biomeId)` (pass-through), `.tickBiomesPerFrame(dt)` (programmatic tick), `.getBiomeTransitionTimer()`, `.getBiomeTransitionDuration()`. `tests/headless/test-phase31.cjs` 95/95 (≈57 static + 38 behavioral: pure module helpers, World API per-region assignment + cross-region difference + within-region stability, color matches `BIOME_DATA`, lerp math + clamping) + `smoke.cjs` 66 Phase 3.1 static-analysis keys + 63 summary keys + `phase31Ok` gate + ACCEPTANCE SUMMARY header update. `tests/gameplay.spec.js` 1 new test (8 sub-checks: initial Forest state, forceBiome echo, transition timer reset, mid-tick midpoint lerp, Crystal Cavern color, biomeLabel for all 8 + Unknown fallback, out-of-range rejection, #biome-info element presence). All earlier phase tests still pass: 1.2 17/17, 1.3 7/7, 1.4 22/22, 1.5 12/12, 1.6 21/21, 1.7 26/26, 2.1 26/26, 2.2 35/35, 2.3 51/51, 2.4 46/46, 2.5 70/70, 2.6 71/71, 2.7 107/107, 2.8 87/87, 3.1 95/95. |
| 3 — World feel | In progress | 3.1 ✅ + 3.2 ✅ + 3.3 ✅ + 3.4 ✅ | Phase 3.1 (Biomes), 3.2 (Stabilizers + collapse), 3.3 (Echoes + HUD counter), 3.4 (Resonance Cores + amplifiers + cost reduction) shipped. Phase 3.5 (Phase Lock + Phase Glider) is up next — see `PHASE_3_5_BRIEF.md`. |
