...
| 9 — Bug bash + hardening | ✅ Done | Working tree | Firefox pointer-lock audio fix (deferred-resume + first-input fallback) + edge case hardening (PhysicsManager y-clamp, World.setBlock GC-safety, PhaseManager setPhase no-op on bad input, reduced-motion skips color pulse + FOV breathing, collapse tick dt clamp) + 57 new headless checks in `tests/headless/test-phase9.cjs` + Firefox pointer-lock Playwright test in `tests/firefox-pointer-lock.spec.js` + Tested-browsers matrix in `README.md` + `KNOWN_ISSUES.md` Platform section updated + "🟫 Discovered in Phase 9.1" section. `§9.4` performance audit was optionally skipped (no perf complaints from §9.1). See `PHASE_9_BRIEF.md`. |
| 10 — Gameplay mechanics, playability & fun pass | ✅ Done (P0 + P1 + P2 fully) | `b6e7dce` | P0 §10.1 → §10.5 + P1 §10.6, §10.7, §10.8, §10.9 + P2 §10.10, §10.11, §10.12, §10.13, §10.14 all shipped. Highlights: §10.1 `PHASE_SHIFT_COST` 5→15 + per-second drain in Beta/Gamma; §10.2 Phase Fuse (3s hold, 30 energy, persistent Memory World); §10.3 collapse = 1 random Echo lost; §10.4 36-Echo biome-sequenced narrative; §10.5 Act 4 Convergence + Nexus finale; §10.6 per-biome signature mechanics (Forest 2× Echoes, Crystal Cavern 2× Cores, Deep Void 2× Glider, Sky Ruins 2× Anchor, Desert unique, Nexus = all); §10.7 dropped LMB/RMB (Path A) + fixed UI labels; §10.8 wired up phase erosion (was dead code); §10.9 energy danger states (HUD throb + audio heartbeat + vignette pulse + 5s Alpha grace); §10.10 Echo Hunter panel (per-biome collected/total breakdown + biome-transition zone overlay); §10.11 wrong-phase Echoes (7 biomes, Phase-Lens-findable, unique lore); §10.12 phase shift preview (0.5s ghost of target phase before commit); §10.13 resonance charge-up (0.5s preview + 1.0s commit + cancel path, cost 15→25); §10.14 New Game+ mode (phase-dominance Fisher-Yates shuffle + ironman flag in pause menu). **482 new headless checks across 13 new test files** (+9 follow-up from a test-debt cleanup). See `PHASE_10_BRIEF.md` + `HANDOFF.md` §Phase 10 closure. |
